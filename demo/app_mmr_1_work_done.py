    ...
    if prompt := st.chat_input("Ask a question about your documents..."):
        ...
        with st.chat_message("assistant"):
            try:
                ...
                print('RagChain initialize start first')
                if use_mmr:
                    print('RagChain initialize start first 2')
                    response_gen = chat.get_mmr_response(prompt, use_web_search)
                    print('RagChain initialize start first 3')
                    sources = chat.get_sources_mmr(prompt, use_web_search) # Problem was there in get_sources_mmr function 
                    print('RagChain initialize start first 4')
