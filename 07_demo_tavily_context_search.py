...
from tools.tavily_search import HybridSearchManager
...
"""
    ...
    vc_manager = VectorStoreManager(embedding_manager= embedding_manager)
    vc_manager.add_documents(chunks)

    print(vc_manager.vector_store)

    tool = HybridSearchManager(vc_manager)

    result = tool.search_mmr(query= question ,use_web_search= True)
    print("\nTool processed successfully")
    print(result)
    print()
    print("\nStarting document fromating.... \n")
    format_result = tool.format_hybrid_context(
        doc_results=result['document_results'],
        web_results= result['web_results']
    )

    print("Document fromated successfully \n")
    print(format_result)

    # print("Program execution RAGchain")
    # rg_manager = RAGchain(vc_manager)
    # result = rg_manager.query_stream_mmr(question=question , k= 5)
    # for i in result:
